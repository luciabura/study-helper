...
from evaluation.perplexity.lm_1b import data_utils
...
CKPT_FILE = bytes(os.path.join(DATA_DIR, './ckpt-*').encode())
GD_FILE = bytes(os.path.join(DATA_DIR, './graph-2016-09-10.pbtxt').encode())
VOCAB_FILE = bytes(os.path.join(DATA_DIR, './vocab-2016-09-10.txt').encode())
...
def _EvalModel(dataset):
  ...
  """
  sess, t = _LoadModel(GD_FILE, CKPT_FILE)
...
def main(unused_argv):
    ...
    dataset = data_utils.LM1BDataset(input("Test file"), vocab)
    _EvalModel(dataset)


def evaluate_average_perplexity():
    vocab = data_utils.CharsVocabulary(VOCAB_FILE, MAX_WORD_LEN)

    dataset = data_utils.LM1BDataset(input("Test file"), vocab)
    perplexity = _EvalModel(dataset)
